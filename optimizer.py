...
# Parameter grid
PARAM_GRID = {
    "score_threshold": [55, 60, 65, 70, 75, 80],
    "take_profit_pct": [0.08, 0.10, 0.12, 0.15, 0.20],
    "stop_loss_pct":   [0.04, 0.05, 0.07, 0.10],
    "max_hold_days":   [2, 3, 4, 5, 7],
    "max_market_cap":  [100_000_000, 300_000_000, 500_000_000, 1_000_000_000, 2_000_000_000, 5_000_000_000],
}
...
def optimize_from_cache(cache_file: str):
    ...
    for threshold, tp, sl, hold in combos:
        ...
        if stats["expectancy"] > best_expectancy and stats["trades"] >= 5:
    ...
    for combo_idx, (threshold, tp, sl, hold, mcap_limit) in enumerate(combos):
        ...
        if (combo_idx + 1) % 50 == 0:
            log.info(f"  [{combo_idx+1}/{len(combos)}] combos tested, "
                     f"best expectancy so far: {best_expectancy:.3f}")
...
def _stats(pnls, tp, sl, threshold, hold, mcap_limit=None):
    import math
    n = len(pnls)
...
def _print_top10(opt_rows, best_combo):
    print("\n" + "=" * 95)
    print("  OPTIMIZER RESULTS — TOP 10 BY EXPECTANCY")
    print("=" * 95)
    sorted_rows = sorted(opt_rows, key=lambda x: x["expectancy"], reverse=True)
    print(f"  {'Threshold':>9} {'MaxMcap':>8} {'TP%':>5} {'SL%':>5} {'Hold':>5} "
          f"{'Trades':>7} {'WinRate':>8} {'AvgPnL':>8} {'Expect':>8} {'ProfitF':>8}")
    print("-" * 95)
    shown = 0
    for r in sorted_rows:
        ...
        print(f"  {r['score_threshold']:>9} {mcap_str:>8} {r['tp_pct']:>5.1f} {r['sl_pct']:>5.1f} "
              ...
              f"{r['win_rate']:>7.1f}% {r['avg_pnl_pct']:>+7.2f}% "
              f"{r['expectancy']:>+7.3f}% {r['profit_factor']:>7.2f}x" if r['profit_factor'] != float('inf')
              else f"{r['expectancy']:>+7.3f}% {'∞':>7}x")
        shown += 1
        if shown >= 10:
            break
    if best_combo:
        mcap_str = f"${best_combo['max_mcap_M']}M" if isinstance(best_combo['max_mcap_M'], int) else best_combo['max_mcap_M']
        print(f"\n  >>> BEST COMBO (by Expectancy, min 5 trades):")
